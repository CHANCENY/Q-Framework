...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login-default-button'])){
   ...
   $check = \Datainterface\Selection::selectById('users', ['mail'=>$data['mail']]);
   if(isset($check[0]['password']) && $check[0]['password'] === $data['password']){
       $_SESSION['private_data']['current_user'] = $check;
       echo \Alerts\Alerts::alert('info', "Your have logged in successfully");
   }else{
      echo \Alerts\Alerts::alert('danger', 'Incorrect username or password');
   }
